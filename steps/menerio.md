...
The installer asks for it itself, stores it locked inside the mission control folder, and
...
old way. One key, stored once, and `mc-menerio-connect` gives every assistant on
the computer the same connection.

**Connecting the notebook and copying the mission control are two separate choices.** Never
present them as one. Somebody whose mission control holds client notes or patient notes can say
...
> every assistant that opens this mission control can save notes there and find them again.
> Connecting sends none of your mission control's files anywhere. Copying them for search is a
...
> 2. Not now. Your mission control is complete without it, and you can come back any time.
...
It needs a mission control that is already on this computer. It does not pull the mission control, does
not check Git, Node or Hermes again, and does not touch the rest of the wiring.
What it does:

1. fetches the kit's small programs, so `mc-menerio-connect` is here
2. fetches `age`, the program that locks the key, if this computer has none
3. asks "Connect Menerio now?" and asks for the key
4. stores the key locked inside the mission control folder
...
7. runs `mc-menerio-connect`, and shows one line for each assistant
8. asks "Copy your mission control's files to Menerio for search?" Enter means no. See below
9. installs the small mission control job, on save and once an hour. It keeps the folder fresh
   from its repository and hands a replaced key to Hermes. It copies to and from
   Menerio only after a yes in step 8

On a mission control that is already connected it does not ask for the key again. It repeats
...
## The second choice: a copy of the mission control, for search

The installer says this, and you should not soften it or shorten it:

> One more choice. Menerio can keep a copy of your mission control's text files, so your assistant
> can search them by meaning and not only by exact word. The copy holds everything in
> your mission control except dev/ and your locked keys. In return, the people and facts Menerio
> holds for you are copied into your mission control's world/ folder as a safety copy. Say yes only
> if you are happy for your mission control's files to be in your Menerio account. Your notebook
> works either way.

The answer is kept for this one computer, as the line `GODSPEED_NOTEBOOK_MIRROR=1` or
`GODSPEED_NOTEBOOK_MIRROR=0` in `~/.godspeed/device.env`. A full install asks once and never
...
- **After a no:** nothing is copied in either direction, and nothing is sent to
  ...
  notes there when they ask it to. `mc-search --local` still searches the mission control's own
  files by word.
- **After a yes:** the mission control's files go up when the mission control saves a version and once an
...
`mc-menerio-connect` prints one line for each assistant it knows: Claude Code,
...
Hermes the new key after the key in the mission control has been replaced.
